(function(namespace) {
    ...
    Game.prototype.checkCactusHit = function() {
        for (var i = 0; i < this.cacti.length; i++) {
            if (this.player.collidesWith(this.cacti[i], this.offset)) {
                this.running = false;
                this.finished = true;
                this.player.wideEyed = true; // Show the player's reaction
                return;
            }
        }
    };
    ...
    Game.prototype.step = function(timestamp) {
        ...
        // Start the game by tapping the screen
    ...
    Game.prototype.resetGame = function() {
        this.cacti = []; // Clear existing cacti
        this.nextCactus = 0;
        this.offset = 0;
        this.running = false;
        this.finished = false;
        this.player = new Dinosaur({ // Reset the player state
            context: this.context, 
            left: 10, 
            bottom: this.canvas.height - GROUND_BUFFER,
            colour: DEFAULT_COLOUR
        });
        this.score = new ScoreBoard({ // Reset the score
            context: this.context, 
            left: this.canvas.width - 10, 
            bottom: 26,
            colour: DEFAULT_COLOUR
